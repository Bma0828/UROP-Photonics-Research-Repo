...
function OptOut = OptimizeDevice(OptParm, dirname)
...
for iter = iterStart:MaxIterations
    ...
    [FigGeo, FigEff] = ShowProgress(OptParm, {xGrid,yGrid}, DispPattern{RobustInd}, iter, MaxIterations, AbsoluteEfficiency, RelativeEfficiency, Figs);
    toc;
end
%filename = fullfile(strcat('\',dirname), strcat(dirname, '.jpg'));
%exportgraphics('Figs', filename);
%saveas('Figs',[pwd strcat('/',dirname)]);
savefig(FigGeo, strcat(dirname, '/', dirname, '_geo.fig'))
savefig(FigEff, strcat(dirname, '/', dirname, '_eff.fig'))
%save(dirname, 'Figs.FigEff')
